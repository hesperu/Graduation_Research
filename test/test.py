...
parser.add_argument("--model",required=False,type=str,default="./output/pix2pix_G_high_epoch_1000.pth",help="使うモデル")
...
for batch_num, data in enumerate(dataloader):
    ...
    out_img = model(data)
    utils.save_image(out_img,pathlib.Path(__file__).parent.joinpath("test_result",str(cnt)+".tiff"),normalize=True)
...
for img_path in range(list(pathlib.Path(__file__).parent.joinpath("test_result").glob("*.tiff"))):
    generated = Image.open(img_path)
    real_ = Image.open(pathlib.Path(__file__).parent.joinpath("IMBRIUM","nac_dtm",img_path.name))
    rmse_res += validate.calc_rmse(generated,real_)

for img_path in range(list(pathlib.Path(__file__).parent.joinpath("test_result").glob("*.tiff"))):
    generated = Image.open(img_path)
    real_ = Image.open(pathlib.Path(__file__).parent.joinpath("IMBRIUM","nac_dtm",img_path.name))
    rmse_res += validate.calc_mae(generated,real_)

rmse_res /= len(list(pathlib.Path(__file__).parent.joinpath("test_result").glob("*.tiff")))
mae_res /= len(list(pathlib.Path(__file__).parent.joinpath("test_result").glob("*.tiff")))
